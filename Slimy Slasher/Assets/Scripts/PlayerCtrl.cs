...
public class PlayerCtrl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (hit.distance <= 0.51f && hit.distance > 0.45f && hit2.distance <= 0.51f && hit2.distance > 0.45f && hit3.distance <= 0.51f && hit3.distance > 0.45f && Time.time > lastHit + 5f) // sets where the player should teleport to if they die
        {
            checkPoint = new Vector2(Mathf.Round(gameObject.transform.position.x) - lastDirection/2, gameObject.transform.position.y);
        }

        if (isHit == true && Time.time >= lastHit + 1f && GetComponent<PlayerHealth>().health > 0) // Changes the player's position after touching spikes
        {
    ...
    void Jump()
    {
        if (Input.GetKeyDown(KeyCode.Space) || Input.GetKeyDown(KeyCode.W) || Input.GetKeyDown(KeyCode.UpArrow) && fixedVelocity == false && start == true)
        {
            RaycastHit2D hit = Physics2D.Raycast(new Vector2(transform.position.x, transform.position.y), Vector2.down, Mathf.Infinity, layerMask);
    ...
    void Dash()
    {
        ...
        {
            if (Input.GetKeyDown(KeyCode.LeftShift) || Input.GetKeyDown(KeyCode.RightShift) && canDash == true && start == true && Time.time >= dash + 1f) // Initiates the dash
            {
            ...
            if (Time.time >= dash + 0.2f && canDash == false) // Ends the dash
            {
                fixedVelocity = false;
